#include <iostream>
#include <vector>
#include <algorithm>
#include <string>
using namespace std;

typedef long double dbl;
typedef long long lng;

// поиск угла 
bool Try(lng a, lng b, lng d, lng e)
{
	int max_angle = 90;
	max_angle++;
	
	vector<dbl> get_sin = {0
		,0.017452406
		,0.034899497
		,0.052335956
		,0.069756474
		,0.087155743
		,0.104528463
		,0.121869343
		,0.139173101
		,0.156434465
		,0.173648178
		,0.190808995
		,0.207911691
		,0.224951054
		,0.241921896
		,0.258819045
		,0.275637356
		,0.292371705
		,0.309016994
		,0.325568154
		,0.342020143
		,0.35836795
		,0.374606593
		,0.390731128
		,0.406736643
		,0.422618262
		,0.438371147
		,0.4539905
		,0.469471563
		,0.48480962
		,0.5
		,0.515038075
		,0.529919264
		,0.544639035
		,0.559192903
		,0.573576436
		,0.587785252
		,0.601815023
		,0.615661475
		,0.629320391
		,0.64278761
		,0.656059029
		,0.669130606
		,0.68199836
		,0.69465837
		,0.707106781
		,0.7193398
		,0.731353702
		,0.743144825
		,0.75470958
		,0.766044443
		,0.777145961
		,0.788010754
		,0.79863551
		,0.809016994
		,0.819152044
		,0.829037573
		,0.838670568
		,0.848048096
		,0.857167301
		,0.866025404
		,0.874619707
		,0.882947593
		,0.891006524
		,0.898794046
		,0.906307787
		,0.913545458
		,0.920504853
		,0.927183855
		,0.933580426
		,0.939692621
		,0.945518576
		,0.951056516
		,0.956304756
		,0.961261696
		,0.965925826
		,0.970295726
		,0.974370065
		,0.978147601
		,0.981627183
		,0.984807753
		,0.987688341
		,0.990268069
		,0.992546152
		,0.994521895
		,0.996194698
		,0.99756405
		,0.998629535
		,0.999390827
		,0.999847695
		,1
	};

	vector<dbl> get_cos = {	
		1
		,0.999847695
		,0.999390827
		,0.998629535
		,0.99756405
		,0.996194698
		,0.994521895
		,0.992546152
		,0.990268069
		,0.987688341
		,0.984807753
		,0.981627183
		,0.978147601
		,0.974370065
		,0.970295726
		,0.965925826
		,0.961261696
		,0.956304756
		,0.951056516
		,0.945518576
		,0.939692621
		,0.933580426
		,0.927183855
		,0.920504853
		,0.913545458
		,0.906307787
		,0.898794046
		,0.891006524
		,0.882947593
		,0.874619707
		,0.866025404
		,0.857167301
		,0.848048096
		,0.838670568
		,0.829037573
		,0.819152044
		,0.809016994
		,0.79863551
		,0.788010754
		,0.777145961
		,0.766044443
		,0.75470958
		,0.743144825
		,0.731353702
		,0.7193398
		,0.707106781
		,0.69465837
		,0.68199836
		,0.669130606
		,0.656059029
		,0.64278761
		,0.629320391
		,0.615661475
		,0.601815023
		,0.587785252
		,0.573576436
		,0.559192903
		,0.544639035
		,0.529919264
		,0.515038075
		,0.5
		,0.48480962
		,0.469471563
		,0.4539905
		,0.438371147
		,0.422618262
		,0.406736643
		,0.390731128
		,0.374606593
		,0.35836795
		,0.342020143
		,0.325568154
		,0.309016994
		,0.292371705
		,0.275637356
		,0.258819045
		,0.241921896
		,0.224951054
		,0.207911691
		,0.190808995
		,0.173648178
		,0.156434465
		,0.139173101
		,0.121869343
		,0.104528463
		,0.087155743
		,0.069756474
		,0.052335956
		,0.034899497
		,0.017452406
		,0	
	};

	// invariant (h<=w) высота меньше ширины
	// a <= b  
	// d <= e  

	vector<dbl> w(max_angle,0); // расчетная ширина при повороте
	vector<dbl> h(max_angle, 0); // расчетная высота при повороте

	// w = b*cos(angle) + a*sin(angle)
	// h = b*sin(angle) + a*cos(angle)

	bool result = false;

	for (int angle = 0; angle < max_angle; angle++)
	{
		w[angle] = b*get_cos[angle] + a*get_sin[angle];
		h[angle] = b*get_sin[angle] + a*get_cos[angle];

		if (d >= h[angle] && e >= w[angle])
		{
			result = true;
			break;
		}
	}

	// todo если дробное значение угла - искать бинарным поиском (до десятой)

	return result;
}

string GetAnswer(lng a, lng b, lng d, lng e)
{	
	if (a<=d && b<=e)
	{
		return "YES";
	}
	
	//dbl x2 = (b * d + a * e)*(b * d + a * e) / (d*d + e*e); 
	//dbl y2 = (b * e + a * d)*(b * e + a * d) / (d*d + e*e); 

	// просовывает строго по диагонали
	if (
	 (d*d*(d*d + e*e) >= (b * d + a * e)*(b * d + a * e)) &&
	 (e*e*(d*d + e*e) >= (b * e + a * d)*(b * e + a * d))
	)
	//if (x2 <= (d*d) && y2 <= (e*e))
	{
		return "YES";
	}

	// просовывает не строго по диагонали
	if (true == Try(a,b,d,e)) 
		return "YES";

	return "NO";
}

//string GetAnswer(dbl a, dbl b, dbl c, dbl d, dbl e)
//{
//	dbl epsilon = 10e-9; // accuracy
//	
//	if (a<(d + epsilon)
//		&& 
//		b<(e+epsilon))
//	{		
//		return "YES";
//	}
//
//	/*
//	// diagonal angle - phy
//	dbl hyp = sqrtl(d*d + e*e);
//
//	dbl sin_phy = d / hyp;
//	dbl cos_phy = e / hyp;
//
//
//	dbl x = b*sin_phy + a*cos_phy; // (b * d + a * e) / sqrtl(d*d + e*e)
//	dbl y = b*cos_phy + a*sin_phy; // (b * e + a * d) / sqrtl(d*d + e*e)
//	*/
//
//	dbl x2 = (b * d + a * e)*(b * d + a * e) / (d*d + e*e); // (b*b*d*d + 2*b*d*a*e + a*a*e*e) / (d*d + e*e)
//	dbl y2 = (b * e + a * d)*(b * e + a * d) / (d*d + e*e); // 
//
//	//if (x <= d && y <= e)
//	if (x2 < (d*d + epsilon) && y2 < (e*e + epsilon))
//	{		
//		return "YES";
//	}
//	return "NO";
//}

// todo template

void swap(dbl&a, dbl&b) { dbl temp = a; a = b; b = temp; }
void swap(lng&a, lng&b) { lng temp = a; a = b; b = temp; }

int main() // WA 20
{	

	_ASSERT(GetAnswer(10, 10, 10, 10) == "YES");
	_ASSERT(GetAnswer(10, 15, 10, 14) == "NO");
	_ASSERT(GetAnswer(10, 15, 10, 15) == "YES");
	_ASSERT(GetAnswer(10, 100, 60, 90) == "NO");
	_ASSERT(GetAnswer(10, 100, 70, 90) == "YES");
	_ASSERT(GetAnswer(15, 29, 14, 100) == "NO");
	_ASSERT(GetAnswer(10, 100, 90, 90) == "YES");
	_ASSERT(GetAnswer(100, 100, 100, 100) == "YES");

	// hole  = 8.8 (e) 6.6 (d)  diag angle 36,9 degrees
	// brick = 10.0 (b) 1.0 (a)
	_ASSERT(GetAnswer(10, 100, 66, 88) == "YES"); // впритирочку но не по диагонали (под углом 35 должно пройти)

	//---
	// pi = 3.141592654

	vector<dbl> v1(3);

	cin >> v1[0] >> v1[1] >> v1[2];
	dbl d, e;
	cin >> d >> e;
	if (d > e) swap(d, e);


	sort(v1.begin(), v1.end());
	dbl a = v1[0];
	dbl b = v1[1];

	string answer = GetAnswer(
		(lng)(10 * a), 
		(lng)(10 * b), 
		(lng)(10 * d), 
		(lng)(10 * e)
		);

	cout << answer;

	return 0;
}